...
export default class SupportController {
    ...
    */
            ...
            async.series([
                ...
                function(done)
                {
                    ...
                    let body = [
                        ...
                        { name: "DATE", value: moment().format('MM-DD-YYYY') },
                        { name: "TIME", value: moment().format('h:mma') },
                        { name: "URL", value: url },
	...
	apiList(req, res, next)
	{
		...
		fs.readdir('../staffline-ver/src/api', function(err, files) 
		{
			let out = [];
			files.forEach( item => {
				out.push("router.use('/', require('../api/"+item+"/"+item+"-api'))")
			})
			res.send(out)
		});
	}
