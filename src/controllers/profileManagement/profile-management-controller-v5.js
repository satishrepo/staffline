...
export default class UserController {
  ...
  */
    ...
    */
    ...
    getMarchingJob(employeeDetailsId)
    {
        return new Promise(resolve => {
            ...
            var options = {
                method: 'POST',
                url: configContainer.jobUrl.matchingJob+employeeDetailsId,
                headers:
                {
            ...
            request(options, function (error, response, rawBody) {
                if(!error && rawBody && rawBody.statusCode == 200){ 
                    totalRecord = rawBody.data.totalRecords;
                }else{
                    commonMethods.catchError('profile-management-controller/getMarchingJob', error);
                }
                return resolve({totalRecord : totalRecord});
            })
        });
    }
            ...
            .then( rs => {
                if(rs)
                {
                    ...
                    if (employeeDetailsId) 
                    {
                        ...
                        promises.push(this.getMarchingJob(employeeDetailsId));

                        Q.all(promises).spread(function (empDetails, allProjects, experiences, documents, skills, application, interview, matchingJobCount) 
                        {
                            ...
                            respData.push({
                                ...
                                interview : interview,
                                matchingJob: matchingJobCount.totalRecord
                            })
