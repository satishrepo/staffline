...
export default class JobsReferralController {
    ...
    */
                    ...
                    async.waterfall([
                        ...
                        function(done) {
                            ...
                            if(resumeObj.uid && !alreadyReferred)
                            {
                                commonMethods.decrypt(resumeObj.uid)
                                .then( dec => {
                                    if(decprofileManagementModelV2)
                                    { 
                                        let userData = dec.split('||'); 
                        ...
                        function (done) {
                            accountModel.getUserByUserName(resumeObj.emailId)
                            .then(user => {
                                // Remove check of user account activation for job apply
                                ...
                                else 
                                {
                                    ...
                                    .then((users) => {
                                        ...
                                        // call Resume-Parser API 
                                        accountModel.parseResume(resumeObj.resumeId, resumeObj.resumeName, resumeObj.resumeFile, 'jobsreferral-controller/applyJobWithoutLogin', resumeObj)
                                            .then(parse => { })
                        ...
                        function (rs, done) {
                            ...
                            .then((data) => {
                                ...
                                if(isNewUser){
                                    ...
                                    //check user already reffer for this job or not
                                    crudOperationModel.findModelByCondition(JobReferral, { candidateEmail: resumeObj.emailId, jobId: resumeObj.jobId } )
                                    .then( chkRefer => {
                                        if(chkRefer) {
                                            ...
                                            crudOperationModel.saveModel(ResumeMaster, 
                                                {employeeReferrerId: chkRefer.referrerResumeId}, 
                                                {resumeId: resumeObj.resumeId})
                                            .then( refer => {
                                            })
                                        }
                                ...
                                let rsp = {
                                    employeeDetailsId: resumeObj.employeeDetailsId,
                                    isNewUser : isNewUser && !inActiveUser, 
    ...
    applyJob(req, res, next) {
        ...
        if (!req.body.jobId) {
            ...
            crudOperationModel.findModelByCondition(ClientJobMaster,
            {
                ...
                {
                    if( req.body.email ){
                        // if request is has email id with auth token then consider this as without login
                        let resumeObj = {
                            employeeDetailsId: 0,
                            jobId: req.body.jobId,
                            firstName: req.body.firstName ? req.body.firstName.trim() : '',
                            lastName: req.body.lastName ? req.body.lastName.trim() : '',
                            phone: req.body.phone,
                            emailId: req.body.email ? req.body.email.trim() : '',
    ...
    sendJobApplyMail(resumeObj, calledFrom, template) {
        ...
        .then(job => {
            if (job.length) 
            {
                ...
                let CONDITIONALMSG = '';
                crudOperationModel.findModelByCondition(UserLoginDetail, 
                    {employeeDetailsId : resumeObj.applicantEmployeeDetailsId, deviceId: { $eq : null}, isDeviceLogin: { $eq : null}})
                .then( userDetail => {
                    if(!userDetail){
                        CONDITIONALMSG = 'Download the StafflinePro™ app to let jobs find you. See relevant job matches, track applications and message your dedicated recruiter for a great job search experience.';
                    }
                    ...
                    if(template && template == enums.emailConfig.codes.jobApplyNewUser.code)
                    {
                        ...
                        body = [
                            ...
                            { name: "CONDITIONALMSG", value: CONDITIONALMSG }
                        ];
                        options = {
                    ...
                    else
                    {
                        ...
                        var options = {
                            method: 'POST',
                            url: configContainer.jobUrl.similarJob+resumeObj.jobId,
                            headers:
                            {
                                'secretKey': configContainer.jobUrl.secretKey
                            },
                            json: true,
                            timeout: 30000
                        };

                        request( options, function (error, exRes, bodyRes) { 
                            if(!error && bodyRes && bodyRes.statusCode == 200){ 
                                totalRecord = bodyRes.data.totalRecords;
                            }else{
                                commonMethods.catchError('jobreferral-controller/sendJobApplyMail v2', error);
                            }
                            ...
                            body = [
                                ...
                                { name: "NumberofMatches", value: totalRecord},
                                { name: "CONDITIONALMSG", value: CONDITIONALMSG}
                            ];
                            let tCodeMatchingJob = totalRecord > 0 ? enums.emailConfig.codes.jobApply.code : enums.emailConfig.codes.jobApplyNoMatchingJob.code
                            options = {
                                mailTemplateCode: tCodeMatchingJob,
                                toMail: [{ mailId: resumeObj.emailId, displayName: resumeObj.firstName, employeeId: resumeObj.employeeDetailsId }],
                                placeHolders: body,
                                replyToEmailid: 'SUPPORTMAILID'
                            };
                ...
                .then(empData => {
                    ...
                    { 
                        ...
                        profileManagementModel.getSourceEntity(source, entity)
                        .then( rs => { 
                            // Email to Recruiter
    ...
    referJobToCandidate(req, res, next)
    {
        ...
        let jobReferralId = 0; //primary key of JobReferral tabel 
        let input = {
            jobId : req.body.jobId,
        ...
        else 
        {
            async.series([
                ...
                function(done)
                {
                    ...
                    .then( rs => { 
                        jobReferralId = rs.dataValues.jobReferralId
                        done()
                    })
                },
                function(done)
                {
                    ...
                    .then( rs => {
                        ...
                        .then( jobInfo => {
                            if(jobInfo.length)
                            {
                                let encKey = commonMethods.encrypt('NOTINTERESTED||'+jobReferralId+'||'+input.jobId+'||'+new Date().getTime());
                                let body = [
                                    ...
                                    { name: "JOBDESCRIPTION", value: jobInfo[0].cjmJobDetails },
                                    { name: "UNIQUECODE", value: encKey},
                                    { name: "CONDITIONALMSG", value: "Download the StafflinePro™ app to let jobs find you. See relevant job matches, track applications and message your dedicated recruiter for a great job search experience."}
                                ];
                                let options = {
                                ...
                                emailModel.mail(options, 'jobsreferral-controller-v2/referJobToCandidate')
                                .then(rs => { })
    ...
    __Backup__sendInviteVaiMail(req, res, next)
    {
        let employeeDetailsId = req.tokenDecoded.data.employeeDetailsId;
        let response = responseFormat.createResponseTemplate(),
            msgCode = [],
                ...
                {   
                    ...
                    {  
                        ...
                        .then(rs => {
                            ...
                            {
                                //default entry
                                item['status'] = enums.contactStatus.alreadyRegistered; //Already registered
                                item['invitationDate'] = null;
                                item.sendMail = false;

                                //if detail exist in employee details then check into resume master
                                crudOperationModel.findModelByCondition(ResumeMaster, { employeeDetailsId : rs.employeeDetailsId })
                                .then(rms => {
                                    if(rms){
                                        // if entry exist in resume master then check into CandidateContact 
                                        crudOperationModel.findModelByCondition(CandidateReferral, { resumeId : rms.resumeId })
                                        .then(rs => {

                                            //if invited user has also refer the friend then what is the status????
                                            if(rs){
                                                item['status'] = enums.contactStatus.referredByOther; //Already referred by other
                                                item['invitationDate'] = null;
                                                item.sendMail = false;
                                                cb();
                                            }else{
                                ...
                                .then(rs => {  
                                    ...
                                    //if user is notexist in CandidateContact (item.contactEmail) then what happen??
                                    //it simple insert with data that comes from frontend status as invited (if send mail true) or null
                                    if(rs && rs.candidateResumeId != resumeId)
                                    { 
                                        if(!rs.status && item.sendMail)
                                        { 
                                        ...
                                        {
                                            item['status'] = enums.contactStatus.invitedByOther;
                                            item['invitationDate'] = null;
                                            item.sendMail = false;
                                        }
                                        cb();
